...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, hparams, version=None, val=False):
        ...
        self.path2 = hparams.training_files or "%s/2-name2text.txt" % exp_dir
        self.path4 = hparams.semantic_dir or "%s/4-cnhubert" % exp_dir
        self.path5 = hparams.wav_dir or "%s/5-wav32k" % exp_dir
        if not os.path.isabs(self.path2):
            self.path2 = os.path.join(exp_dir, "..", self.path2)
        if not os.path.isabs(self.path4):
            self.path4 = os.path.join(exp_dir, "..", self.path4)
        if not os.path.isabs(self.path5):
            self.path5 = os.path.join(exp_dir, "..", self.path5)
        self.path2 = os.path.normpath(self.path2)
        self.path4 = os.path.normpath(self.path4)
        self.path5 = os.path.normpath(self.path5)
        assert os.path.exists(self.path2), f"Missing: {self.path2}"
        assert os.path.exists(self.path4), f"Missing: {self.path4}"
        assert os.path.exists(self.path5), f"Missing: {self.path5}"
        self.is_v2Pro = version in {"v2Pro", "v2ProPlus"}
        if self.is_v2Pro:
            self.path7 = "%s/7-sv_cn" % exp_dir
            assert os.path.exists(self.path7), f"Missing: {self.path7}"
