...
def savee(ckpt, name, epoch, steps, hps, model_version=None, lora_rank=None):
    try:
        ...
        save_dir = getattr(hps, "save_weight_dir", None) or getattr(hps, "s2_ckpt_dir", None) or hps.data.exp_dir
        if lora_rank:
            opt["lora_rank"] = lora_rank
            my_save2(opt, "%s/%s.pth" % (save_dir, name), model_version)
        elif model_version != None and "Pro" in model_version:
            my_save2(opt, "%s/%s.pth" % (save_dir, name), model_version)
        else:
            my_save(opt, "%s/%s.pth" % (save_dir, name))
        return "Success."
    except:
