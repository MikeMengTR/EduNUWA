...
cnhubert_base_path = os.environ.get(
    "cnhubert_base_path",
    os.path.join(_base_dir, "pretrained_models", "chinese-hubert-base")
)
bert_path = os.environ.get(
    "bert_path",
    os.path.join(_base_dir, "pretrained_models", "chinese-roberta-wwm-ext-large")
)
...
def change_sovits_weights(sovits_path, prompt_language=None, text_language=None):
    ...
    # Initialize default values for yield variables (used when called outside Gradio)
    prompt_text_update = {"__type__": "update"}
    prompt_language_update = {"__type__": "update", "value": i18n("中文")}
    text_update = {"__type__": "update"}
    text_language_update = {"__type__": "update", "value": i18n("中文")}
    visible_sample_steps = False if model_version not in v3v4set else True
    visible_inp_refs = True if model_version not in v3v4set else False
    ...
    if not getattr(hps.model, "version", None):
...
try:
    gen = change_sovits_weights(sovits_path)
    for _ in gen:
        pass
except:
    traceback.print_exc()
